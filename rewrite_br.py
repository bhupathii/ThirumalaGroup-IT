...
import { Printer, ArrowLeft, FileText, ChevronRight } from 'lucide-react';
...
const BusinessReport: React.FC = () => {
  ...
  const fetchBusinessData = async () => {
    ...
    try {
      ...
      const { data: rawTxs } = await supabase.from('finance_cashbook_entries').select('*');

      const filteredLoans = allLoans.filter(l => {
        if (l.date < startDate || l.date > endDate) return false;
        if (targetPartner && l.customer?.partner_name !== targetPartner.name) return false;
        return true;
      });
      ...
      filteredLoans.forEach(l => {
        ...
        const loanTxs = (rawTxs || []).filter(t => t.loan_id === l.id);
        const colList = loanTxs.filter(t => t.type === 'Collection');
        ...
        colList.forEach(c => {
           // In Cashbook entries, the interest and penalty might be stored differently or we can approximate
           // Wait, usually the 'remarks' or 'description' holds this, but let's check how other modules do it.
           // Since we don't have split in finance_cashbook_entries easily available without parsing, let's use the 'dues' array for pending and calculate received from total paid minus principal.
           collectedPrincipal += Number(c.amount);
        });

        const outstandingPrincipal = Math.max(0, p - collectedPrincipal);
        pOut += outstandingPrincipal;

        const loanDues = dues.filter(d => d.loanId === l.id);
        let loanPInt = 0;
        let loanPPen = 0;
        let loanTDue = 0;

        loanDues.forEach(d => {
           loanPInt += d.interestPending;
           loanPPen += d.penaltyPending;
           loanTDue += d.totalPending;
        });
    ...
    <div className="space-y-6 max-w-[1400px] mx-auto p-6 print:p-0">
      ...
      <div className={`grid grid-cols-2 md:grid-cols-4 gap-4 ${showPrintPreview ? 'print:hidden' : ''}`}>
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm px-4 py-3 flex flex-col justify-center">
          ...
          <input type="date" value={startDate} onChange={e => setStartDate(e.target.value)} className="w-full text-slate-900 bg-transparent border-none p-0 focus:ring-0 cursor-pointer finance-sidebar-link uppercase" />
        </div>
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm px-4 py-3 flex flex-col justify-center">
          <label className="text-slate-400 mb-1 finance-small-label uppercase">To Date</label>
          <input type="date" value={endDate} onChange={e => setEndDate(e.target.value)} className="w-full text-slate-900 bg-transparent border-none p-0 focus:ring-0 cursor-pointer finance-sidebar-link uppercase" />
        </div>
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm px-4 py-3 flex flex-col justify-center">
          <label className="text-slate-400 mb-1 finance-small-label uppercase">Partner Filter</label>
          <select value={selectedPartnerId} onChange={e => setSelectedPartnerId(e.target.value)} className="w-full bg-transparent border-none p-0 text-[#0b1329] font-black focus:ring-0 finance-h1 uppercase">
              ...
              <table className="w-full text-left border-collapse finance-caption">
                <thead>
                  <tr className="bg-white border-b border-slate-200 text-slate-400 text-[10px]">
                    ...
                    <th className="px-3 py-2 uppercase font-bold text-right">Principal</th>
                    <th className="px-3 py-2 uppercase font-bold text-right text-emerald-600">Int Recv</th>
                    <th className="px-3 py-2 uppercase font-bold text-right text-emerald-600">Pen Recv</th>
                    <th className="px-3 py-2 uppercase font-bold text-right text-rose-500">Pend Int</th>
                    <th className="px-3 py-2 uppercase font-bold text-right text-rose-500">Pend Pen</th>
                    <th className="px-3 py-2 uppercase font-bold text-right text-rose-700">Present Due</th>
                    <th className="px-3 py-2 uppercase font-bold text-center">Status</th>
                ...
                <tbody className="divide-y divide-slate-100">
                  {loans.length === 0 ? (
                    ...
                    loans.sort((a,b) => sortNumerically(a.cdNumber, b.cdNumber)).map(row => (
                      <tr 
                        key={row.loanId} 
                        className="hover:bg-slate-50/50 transition-colors cursor-pointer"
                        onClick={() => navigate(`/finance/cd-ledger?loanId=${row.loanId}`)}
                      >
                        <td className="px-3 py-2.5 font-mono text-slate-900 font-black">{row.cdNumber}</td>
                        <td className="px-3 py-2.5 text-slate-900 font-bold uppercase truncate max-w-[150px]">{row.borrower}</td>
                        <td className="px-3 py-2.5 text-right text-slate-900 font-medium">₹{row.principal.toLocaleString('en-IN')}</td>
                        <td className="px-3 py-2.5 text-right text-emerald-650 font-bold">₹{Math.round(row.interestReceived).toLocaleString('en-IN')}</td>
                        <td className="px-3 py-2.5 text-right text-emerald-600">₹{Math.round(row.penaltyReceived).toLocaleString('en-IN')}</td>
                        <td className="px-3 py-2.5 text-right text-rose-500">₹{Math.round(row.pendingInterest).toLocaleString('en-IN')}</td>
                        <td className="px-3 py-2.5 text-right text-rose-500">₹{Math.round(row.pendingPenalty).toLocaleString('en-IN')}</td>
                        <td className="px-3 py-2.5 text-right text-rose-700 font-black">₹{Math.round(row.presentDue).toLocaleString('en-IN')}</td>
                        <td className="px-3 py-2.5 text-center">
                          <span className={`px-1.5 py-0.5 rounded text-[9px] font-bold uppercase ${row.status === 'Active' ? 'bg-emerald-50 text-emerald-700 border border-emerald-250' : 'bg-slate-150 text-slate-600'}`}>
                            {row.status}
                          </span>
                        </td>
                        <td className="px-3 py-2.5 text-slate-400 text-center"><ChevronRight className="w-4 h-4" /></td>
                      </tr>
                    ))
                  )}
                </tbody>
        ...
        <div className="space-y-6 pb-12">
          <div className="flex justify-between items-end border-b border-slate-950 pb-2">
            <div>
              ...
              <p className="text-[13px] uppercase text-slate-500">Business Details & Portfolio Overview</p>
            </div>
          ...
          <table className="w-full text-left border-collapse border border-slate-300">
            <thead>
              <tr className="bg-slate-100 border-b border-slate-300">
                <th className="px-2 py-1.5 border-r border-slate-300 font-bold">CD No</th>
                <th className="px-2 py-1.5 border-r border-slate-300 font-bold">Borrower</th>
                <th className="px-2 py-1.5 border-r border-slate-300 font-bold text-right">Principal</th>
                <th className="px-2 py-1.5 border-r border-slate-300 font-bold text-right">Int Recv</th>
                <th className="px-2 py-1.5 border-r border-slate-300 font-bold text-right">Pen Recv</th>
                <th className="px-2 py-1.5 border-r border-slate-300 font-bold text-right">Pend Int</th>
                <th className="px-2 py-1.5 border-r border-slate-300 font-bold text-right">Pend Pen</th>
                <th className="px-2 py-1.5 border-r border-slate-300 font-bold text-right">Present Due</th>
                <th className="px-2 py-1.5 border-slate-300 font-bold text-center">Status</th>
              </tr>
            </thead>
            <tbody>
              {loans.sort((a,b) => sortNumerically(a.cdNumber, b.cdNumber)).map(row => (
                <tr key={row.loanId} className="border-b border-slate-200">
                  <td className="px-2 py-1 border-r border-slate-300 font-bold">{row.cdNumber}</td>
                  <td className="px-2 py-1 border-r border-slate-300 uppercase truncate max-w-[150px]">{row.borrower}</td>
                  <td className="px-2 py-1 border-r border-slate-300 text-right">₹{row.principal.toLocaleString('en-IN')}</td>
                  <td className="px-2 py-1 border-r border-slate-300 text-right">₹{Math.round(row.interestReceived).toLocaleString('en-IN')}</td>
                  <td className="px-2 py-1 border-r border-slate-300 text-right">₹{Math.round(row.penaltyReceived).toLocaleString('en-IN')}</td>
                  <td className="px-2 py-1 border-r border-slate-300 text-right">₹{Math.round(row.pendingInterest).toLocaleString('en-IN')}</td>
                  <td className="px-2 py-1 border-r border-slate-300 text-right">₹{Math.round(row.pendingPenalty).toLocaleString('en-IN')}</td>
                  <td className="px-2 py-1 border-r border-slate-300 text-right font-black">₹{Math.round(row.presentDue).toLocaleString('en-IN')}</td>
