...
const STBDLedger: React.FC = () => {
  ...
  const fetchLedgerData = async () => {
    ...
    try {
      ...
      const stbdLoans = loans.filter(l => 
        l.loan_category === 'STBD' || 
        l.loan_id.toUpperCase().startsWith('STBD')
      );

      const rows = stbdLoans.map(loan => {
        const loanWithTxs = { ...loan, transactions: txs.filter(t => t.loan_id === loan.id) };
        const calc = financeCalculationService.getLoanCalculations(loanWithTxs, settings['STBD'] || null);

        // Get installments info
        const amount = Number(loan.amount);
        const period = Number(loan.duration_months);
        const capitalizedInterest = amount * (Number(loan.interest_rate) / 100) * period;
        const totalLoan = amount + capitalizedInterest;
        const balanceWith = Number(loan.balance_with_interest ?? totalLoan);
  ...
  const openLoanDetails = async (loan: any) => {
    ...
    // Default installments to pay based on installments due
  ...
  const getInstallmentSchedule = (loan: any) => {
    ...
    const installmentAmount = Number(loan.rawLoan.due_amount || (loan.totalRepayable / duration));
  ...
  const getPayableCalculations = () => {
    ...
    const instAmt = Number(selectedLoan.rawLoan.due_amount || (selectedLoan.totalRepayable / selectedLoan.duration));
    const ipaid = Number(selectedLoan.installmentsPaid || 0);
    
    // Calculate penalty using service helper
    const calculatedPenalty = financeCalculationService.calculateSTBDPenalty(
      instAmt,
      ipaid,
      selectedLoan.date,
      paymentDate,
    ...
    // Split computations
    const splits = financeCalculationService.computeSTBDPaymentSplit(
  ...
  const postPayment = async () => {
    ...
    try {
      ...
      const res = await supabaseFinance.postStbdLedgerPayment({
        loanId: selectedLoan.id,
        customerId: selectedLoan.rawLoan.customer_id,
        customerName: selectedLoan.customerName,
        loanIdStr: selectedLoan.loanId,
        userName: 'Cashier', // Replace with active user context if available
        payingInsts,
  ...
  return (
    <div className="space-y-6 p-6 max-w-7xl mx-auto print:p-0">
            ...
            <table className="min-w-full divide-y divide-gray-200 md:text-sm finance-caption">
              <thead>
                <tr className="bg-slate-50">
                  ...
                  <th className="text-right finance-small-label uppercase">Balance (Dr)</th>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredRows.map(row => (
                  <tr key={row.id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-3 py-3 text-emerald-900 font-mono font-semibold">{row.loanId}</td>
                    ...
                    <td className="px-3 py-3 text-right text-gray-500">
                      {row.duration} months <span className="text-[10px]">({row.interestRate}%)</span>
                    </td>
                    <td className="px-3 py-3 text-right text-gray-900">
                    ...
                    <td className="px-3 py-3 text-center">
                      <button 
                        ...
                        className="inline-flex items-center text-emerald-600 hover:text-emerald-800 hover:underline text-xs font-semibold gap-1"
                      >
      ...
      {isDrawerOpen && selectedLoan && (
        <div className="fixed inset-0 z-50 overflow-hidden bg-gray-900/40 backdrop-blur-sm flex justify-end">
          <div className="w-full max-w-4xl bg-white shadow-2xl h-full flex flex-col animate-slide-in">
            {/* Drawer Header */}
            <div className="px-6 py-4 bg-gradient-to-r from-emerald-800 to-teal-800 text-white flex justify-between items-center">
              <div>
                <h2 className="text-xl font-bold font-sans">Account Ledger: {selectedLoan.loanId}</h2>
                <p className="text-emerald-100 text-xs uppercase tracking-wider">{selectedLoan.customerName}</p>
              </div>
              <div className="flex items-center gap-3">
                ...
                <button 
                  ...
                  className="p-1 rounded-full hover:bg-emerald-700/50 text-white transition-colors"
                >
                  <X className="w-6 h-6" />
                </button>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                ...
                <div className="bg-slate-50 border p-3 rounded-xl">
                  <div className="text-gray-400 text-xs uppercase font-semibold">Capital Interest (Flat)</div>
                  <div className="text-lg font-bold text-emerald-700 mt-1">₹{selectedLoan.interestAmount.toLocaleString('en-IN')}</div>
                </div>
                <div className="bg-slate-50 border p-3 rounded-xl">
                  <div className="text-gray-400 text-xs uppercase font-semibold">Installments Paid</div>
                  <div className="text-lg font-bold text-teal-700 mt-1">{selectedLoan.installmentsPaid} / {selectedLoan.duration}</div>
                </div>
                <div className="bg-slate-50 border p-3 rounded-xl">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
                ...
                <div className="lg:col-span-3 space-y-3">
                  <h3 className="text-sm font-bold text-gray-900 border-b pb-1">Installments Schedule</h3>
                ...
                <div className="lg:col-span-2 bg-slate-50 p-4 border rounded-2xl space-y-4">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="bg-emerald-50 p-2.5 rounded-xl border border-emerald-100 text-[10px] text-emerald-800 space-y-1">
                      <div className="font-bold uppercase tracking-wider text-emerald-900 border-b pb-0.5 mb-1 text-[9px]">Credits Split Allocation Preview:</div>
                      <div className="flex justify-between">
                        <span>STBD A/c (Principal Component):</span>
                        <span className="font-semibold">₹{payCalcs.principalPaid}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>STBD Commission A/c (Interest):</span>
                        <span className="font-semibold">₹{payCalcs.commissionPaid}</span>
                      </div>
                      {payCalcs.actualPenaltyPaid > 0 && (
                        <div className="flex justify-between">
                          <span>STBD PENALTY A/c (Penalty):</span>
                    ...
                    <Button 
                      onClick={postPayment}
                      variant="primary" 
                      className="w-full text-center py-2 text-sm font-semibold rounded-xl bg-emerald-700 hover:bg-emerald-800 text-white"
                      ...
                      <tbody className="bg-white divide-y divide-gray-100">
                        {selectedLoan.rawLoan.transactions.map((tx: any) => (
                          <tr key={tx.id} className="hover:bg-slate-50">
                            ...
                            <td className="px-3 py-2 font-mono text-emerald-800 font-medium">{tx.receipt_no || 'N/A'}</td>
      ...
      <FinancePrintPreview
        ...
        title="STBD Ledger"
        documentTitle="STBD LEDGER REPORT"
      >
        {filteredRows.length > 0 && (
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200 md:text-sm finance-caption">
          ...
          <div className="bg-white rounded-3xl p-6 w-full max-w-lg shadow-xl relative text-left">
            ...
            <div className="space-y-4">
              <div className="p-4 bg-orange-50 text-orange-850 border border-orange-200 rounded-2xl text-xs leading-normal">
                ...
                <p>Are you sure you want to close this STBD account under NPA? This will mark the loan status as Closed with NPA designation and record the settlement and waived amounts.</p>
              </div>
              <div>
                <Input
