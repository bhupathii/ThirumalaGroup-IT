...
const CDLedger: React.FC = () => {
  ...
  const fetchLedgerData = async () => {
    ...
    try {
      ...
      // Filter for Chit Fund / Cash Deposit prefix "CD" or category "CD"
      const cdLoans = loans.filter(l => 
        l.loan_category === 'CD' || 
        l.loan_id.toUpperCase().startsWith('CD')
      );

      const rows = cdLoans.map(loan => {
      ...
      console.error(err);
      toast.error('Failed to compile Chit Fund (CD) Ledger');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6 p-6 max-w-7xl mx-auto print:p-0">
      <div className="flex justify-between items-center border-b border-green-100 pb-4 print:hidden">
        <div>
          <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight">CD (Chit Fund) Ledger</h1>
          <p className="text-gray-500 text-sm mt-1">Review accounts starting with the 'CD' identifier prefix</p>
        </div>
      ...
      <Card title="Chit Fund Ledger Index" subtitle="CD category receivable accounts summary" className="shadow-md">
        {loading ? (
          <div className="flex justify-center py-12">
            <div className="animate-spin rounded-full h-10 w-10 border-t-2 border-green-500"></div>
          </div>
        ) : filteredRows.length === 0 ? (
          <div className="text-center py-8 text-gray-400">No CD ledger entries found</div>
        ) : (
