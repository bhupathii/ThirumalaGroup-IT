    ...
    <div className="space-y-6 p-6 max-w-7xl mx-auto select-none">
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-slate-400 border border-dashed rounded bg-slate-50">
          ...
          <p className="font-semibold text-slate-650">No operators registered</p>
              ...
              {users.map(u => (
                ...
                >
                  ...
                  <span className={`text-[10px] px-2 py-0.5 rounded font-mono font-bold capitalize ${
                    selectedUserId === u.id ? 'bg-slate-800 text-white' : 'bg-slate-100 text-slate-700'
                  }`}>
                    {u.userType}
                  </span>
                </div>
          ...
          >
            <div className="space-y-6">
              ...
              <div className="flex justify-between items-center bg-slate-50 p-2.5 rounded-lg border border-slate-150 text-xs">
                <span className="font-bold text-slate-400 uppercase tracking-wider text-[10px]">Quick Config:</span>
                <div className="flex gap-2">
                  <button onClick={handleSelectAll} className="text-slate-800 font-bold hover:underline">Select All</button>
                  <span className="text-slate-350">|</span>
                  <button onClick={handleClearAll} className="text-red-650 font-bold hover:underline">Clear All</button>
                </div>
              </div>
              ...
              <div className="flex justify-end pt-4 border-t border-slate-100">
                <Button onClick={handleSave} variant="success" icon={Save} disabled={saving} className="bg-[#0b1329] border-[#0b1329] hover:bg-slate-800 text-white font-bold text-xs rounded-lg px-4 py-2">
